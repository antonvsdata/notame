...
#' conover_res <- perform_conover_test(drop_qcs(example_set), "Feature ~ Time", separate_by = "Group")
#' @export
perform_conover_test <- function(object, formula_char, all_features = FALSE, separate_by = NULL) {
  if (length(all.vars(as.formula(formula_char))) != 2) {
    stop("Only one-way formulas are supported for Conover-Iman tests.")
  }
  ...
  test_fun <- function(feature, formula, data) {
    ...
    tryCatch(
      {
        conover <- PMCMRplus::kwAllPairsConoverTest(formula, data, p.adjust.method = "none")
        result_row <- data.frame(
          Feature_ID = feature,
          stringsAsFactors = FALSE
        )
        ...
        for (i in seq_len(ncol(p_vals))) {
          for (j in seq_len(nrow(p_vals))) {
            ...
            prefix <- paste0(ind_var, var1, "vs", var2, "_")
