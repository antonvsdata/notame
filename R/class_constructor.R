...
check_pheno_data <- function(x, id_prefix) {
  ...
  if (!"Sample_ID" %in% colnames(x)) {
    ...
    if (any(is.na(x$Sample_ID))) {
      log_text("Missing values found in Sample_ID, filling missing sample IDs of QC samples.")
      x$Sample_ID[is.na(x$Sample_ID) & x$QC == "QC"] <- "QC"
      if (any(is.na(x$Sample_ID))) {
        stop("Missing values in Sample_ID after filling IDs of QCs")
      }
    }
...
name_features <- function(feature_data) {
  ...
  # Keep adding significant numbers until all feature IDs are unique
  feature_data$Feature_ID <- feature_data$Split
  digits <- 3
  while (anyDuplicated(feature_data$Feature_ID) & digits < 11) {
    # Concatenate rounded mass and retention time
    round_mz <- as.numeric(feature_data[, mz_col]) %>% signif(digits = digits) %>%
      as.character() %>% gsub("[.]", "_", .)
    round_rt <- as.numeric(feature_data[, rt_col]) %>% signif(digits = digits) %>%
      as.character() %>% gsub("[.]", "_", .)
  ...
  if (anyDuplicated(feature_data$Feature_ID)) {
    duplicates <- paste0(feature_data$Feature_ID[duplicated(feature_data$Feature_ID)],
                         collapse = ", ")
    stop(paste0("Could not create unique feature names from m/z and retention time columns, using 10 significant digits. Duplicated values: ",
                duplicates))
  }
