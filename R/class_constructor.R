...
check_pheno_data <- function(x, id_prefix) {
  ...
  if (!"Sample_ID" %in% colnames(x)) {
    ...
    log_text(paste0("Sample ID autogenerated from injection orders and prefix ", id_prefix))
  } else {
    ...
    if (any(is.na(x$Sample_ID))) {
      log_text("Missing values found in Sample_ID after filling IDs of QCs, trying to detect 'Blank samples'")
    ...
    if (sum(x$Sample_ID == "QC")) {
      log_text("Adding running index to 'QC' sample IDs")
    ...
    if (sum(x$Sample_ID == "Blank")) {
      log_text("Adding running index to 'Blank' sample IDs")
      x$Sample_ID[x$Sample_ID == "Blank"] <- paste0("Blank_", seq_len(sum(x$Sample_ID == "Blank")))
    }
...
check_exprs <- function(exprs_) {
  log_text("Checking that feature abundaces only contain numeric values")
...
check_feature_data <- function(feature_data) {
  log_text("Checking that feature IDs are unique and not stored as numbers")
