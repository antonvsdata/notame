...
check_pheno_data <- function(x, id_prefix, id_column = NULL, log_messages = FALSE) {
  ...
  if (!is.null(id_column) && !any(duplicated(x[, id_column])) && !any(is.na(x[, id_column]))) {
    colnames(x)[colnames(x) == id_column] <- "Sample_ID"
    log_text_if(paste("Column", id_column, 'changed to "Sample_ID"'), log_messages)
  } else {
    log_text_if("Provided sample ID is not valid", log_messages)
  }
...
check_feature_data <- function(feature_data, mz_values = c(10, 2000), rt_values = c(0, 20), log_messages = FALSE) {
  ...
  if (!all(dplyr::between(mz, mz_values[1], mz_values[2])) || !all(dplyr::between(rt, rt_values[1], rt_values[2]))) {
...
#' @param mz_values numeric vector of two, all m/z values should be in between these
#' @param rt_values numeric vector of two, all retention time values should be in between these
...
read_from_excel <- function(file, sheet = 1, id_column = NULL, corner_row = NULL, corner_column = NULL,
                            id_prefix = "ID_", split_by = NULL, name = NULL, mz_values = c(10, 2000), rt_values = c(0, 20),
                            skip_checks = FALSE) {
  ...
  dada <- openxlsx::read.xlsx(file, sheet, colNames = FALSE, sep.names = "_")
  ...
  if (!is.null(name)) {
    ...
    if (!all(split_by %in% colnames(feature_data))) {
      stop(paste0("Couldn't find column(s): ",
                  paste(split_by[!(split_by %in% colnames(feature_data))], collapse = ", ")))
    }
  ...
  if (!skip_checks) {
    ...
    feature_data <- check_feature_data(feature_data, mz_values = mz_values, rt_values = rt_values, log_messages = TRUE)
  }
