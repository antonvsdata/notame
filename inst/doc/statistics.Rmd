...
- ```mixomics_plsda``` fits classic PLS-DA  
- ```mixomics_plsda_optimize``` finds the optimal number of components for PLS-DA  
- ```mixomics_splsda_optimize``` fits sPLS-DA, which uses a clever variable selection scheme to select the optimal number of components AND the optimal number of features for each component. Try it out!
